...
class LungCancerCtDataset(Dataset):
    def __init__(self, csv_file, train=True, transform=None, train_split=0.8):
        self.data = pd.read_csv(csv_file)
        self.transform = transform
        
        # Map string labels to integers
        self.label_map = {'adenocarcinoma': 0, 'large_cell_carcinoma': 1, 'normal': 2, 'squamouscell_carcinoma': 3}
...
def get_lung_cancer_ct(csv_file, train=True, batch_size=params.batch_size):
        """Get Lung Cancer dataset ct scan loader."""
        ...
        dataset = LungCancerCtDataset(csv_file=csv_file, train=train, transform=transform)
